...
一次寫入太多步數時，有些讀取 Health Connect 的 App 會忽略那筆紀錄。手動模式的每筆
紀錄最多涵蓋 60 秒，所以按下 10,000 等於宣稱一分鐘內走了一萬步。實測 Google Fit
不會把這種紀錄算進它的步數，但不是每個 App 都這樣，有的照收。自動模式沒有這個問題，
它照你設定的速率把步數鋪開——實測每分鐘 600 步以內，Fit 正常累加。
